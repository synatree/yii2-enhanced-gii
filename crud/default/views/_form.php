...
foreach ($relations as $name => $rel) {
    ...
    if ($rel[$generator::REL_IS_MULTIPLE] && isset($rel[$generator::REL_TABLE]) && !in_array($name, $generator->skippedRelations)) {
        $forms .= "        [\n".
            "            'label' => '<i class=\"". $generator->generateIconClass($rel[$generator::REL_CLASS])."\"></i> ' . Html::encode(".$generator->generateString($generator->pluralize ? Inflector::pluralize($rel[$generator::REL_CLASS]) : $rel[$generator::REL_CLASS])."),\n".
